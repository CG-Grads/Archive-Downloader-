  ...
  for obs_row in $(seq 2 $data) #i; obs indice
	do
  		...
  		obsid_d=$(sed -n "${obs_row}"p "${obsid_name}" | cut -b 2)
 	  	...
 	  	part_date=$(grep -Fn "${obsid_full}" ${tmp_location}/$master_catalog)
 	  	part_date=${part_date%:*}
 	  	part_date=${part_date: -16:7}
 	  	part_date=${part_date/-/_}


					url1="https://heasarc.gsfc.nasa.gov/FTP/swift/data/obs/${part_date}//${obsid_full}/."
    				...
    				echo "wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R 'index*' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/swift/data/obs/${part_date}//${obsid_full}/. -P ${save_location}/${obsid_full}_downloading" >> ${download_script_path}
    				...
    				echo 'size=$(du -ch '$save_location'/'$obsid_full'/ | tail -1 | cut -f1)' >> ${download_script_path}
    				...
    				echo 'nof=$(ls '$save_location'/'$obsid_full'/ | wc -l)' >> ${download_script_path}
						echo 'nof_2=$(ls '$save_location'/'$obsid_full'/* | wc -l)' >> ${download_script_path}
	...
	cut -d '|' -f 8 ${tmp_location}/${master_catalog} > ${tmp}/tmp_stat.txt
