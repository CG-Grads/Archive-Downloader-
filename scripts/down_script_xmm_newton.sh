	...
	do
  		...
  		obsid_d=$(sed -n "${obs_row}"p "${obsid_name}" | cut -b 2)
 	  	obsid_g=$(sed -n "${obs_row}"p "${obsid_name}" | cut -b 1-2)
					...
					url1="https://heasarc.gsfc.nasa.gov/FTP/xmm/data/rev0//${obsid_full}/."
    				...
    				echo "wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R 'index*' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/xmm/data/rev0//${obsid_full}/. -P ${save_location}/${obsid_full}_downloading" >> ${download_script_path}
